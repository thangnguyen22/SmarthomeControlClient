...
public class CustomClock extends View {
	...
	public static final int COLOR_WHEEL_STROKE_WIDTH_DEF_VALUE = 2;
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawCircle(pointerPosition[0], pointerPosition[1],
				(float) (mPointerRadius * 1.2), mPointerHaloPaint);

		// Draw the pointer (the currently selected color) slightly smaller on
		// top.
		canvas.drawCircle(pointerPosition[0], pointerPosition[1],
				(float) (mPointerRadius / 2.0), mPointerColor);
...
*/
				...
				if (mUserIsMovingPointer) {
					...
					if (last_radians > radians && radians < (360 / 6) && x > lastX
						...
						if (!block_end && !block_start)
							block_end = false; //Khong dung lai khi da ket thuc
						// if (block_start)
						// block_start = false;
						...
						if (!block_start && !block_end)
							block_start = false; //Tiếp tục lùi khi đã về đến 0
						// if (block_end)
						// block_end = false;
	...
	public interface OnCircleSeekBarChangeListener {
		...
		void onProgressChanged(CustomClock seekBar,
